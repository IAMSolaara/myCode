...
public class Lista {
    ...
    public boolean inserisciOrdinato(Merce m) {
        boolean out = true;
        Node tmp = head;
        if (m != null) {
            if (head == null ||
                    tmp.getMerce().getNome().compareTo(m.getNome()) > 0) { //lista vuota oppure m viene prima della testa
                push(m);
            }
            else { //lista non vuota e m dopo la testa
                while(  tmp.getNext() != null 
                        && tmp.getNext().getMerce().getNome().compareTo(m.getNome()) < 0) { //compreso tra primo e ultimo
                            tmp = tmp.getNext();
                        }
                Node n = new Node(m.getNome(), m.getPrezzo());
                n.setNext(tmp.getNext());
                tmp.setNext(n);
                //(voce di duke nukem): aah, much better
            }
